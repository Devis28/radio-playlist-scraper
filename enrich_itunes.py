# enrich_itunes.py
# Primárny zdroj: iTunes (year, duration_ms, genre, album, track_number)
# Fallback: MusicBrainz pre chýbajúce polia + artist_country_code (ISO)
# Chýbajúce údaje -> "Not found". Bez poľa "duration" (MM:SS).
...
# iTunes cache (alias --cache kvôli spätné kompatibilite)
DEFAULT_ITUNES_CACHE = os.path.join("data", "enrich_cache.json")
DEFAULT_MB_CACHE = os.path.join("data", "mb_cache.json")
...
ITUNES_SLEEP_DEFAULT = 0.6
MB_LIMIT_DEFAULT = 30
MB_SLEEP_DEFAULT = 1.1
...
def _update_if_missing(item: dict, meta: dict, keys) -> Tuple[bool, bool]:
    """
    Zapíše meta[key] do item, len ak je prázdny/Not found.
    Vracia (changed_any, changed_real) – changed_real len ak sme zapísali reálnu hodnotu (nie "Not found").
    """
    changed = False
    changed_real = False
    for k in keys:
...
def _mb_get(path: str, headers: dict, params: dict) -> Optional[dict]:
    ...
    try:
        ...
        time.sleep(_mb_get.MB_SLEEP)  # rate limit ~1 req/s
    return data
_mb_get.MB_SLEEP = MB_SLEEP_DEFAULT  # dá sa upraviť z CLI
...
def mb_fetch_artist_area_code(artist_mbid: str, headers: dict) -> str:
    # artist/{id}?inc=area+begin-area → ISO kód krajiny interpreta
...
def _mb_genre_from_recording(rec: dict) -> Optional[str]:
    # preferuj "genres" (novšie), inak najpopulárnejší "tag"
...
def mb_lookup_fallback(artist: str, title: str, headers: dict) -> Optional[dict]:
    """
    Nájde recording → najskoršie vydanie (album názov, rok),
    doplní žáner, dĺžku (ms) a track_number (z release detailu), + artist_country_code.
    """
...
def enrich_items(items: list, it_cache: dict, mb_cache: dict,
    ...
    """
    iTunes → vyplň čo sa dá; ak niečo chýba, MB fallback + artist_country_code.
    ...
    for it in items:
        ...
        # zabezpeč "Not found" pre chýbajúce polia
...
def main():
    ...
    # iTunes cache + spätná kompatibilita
    ap.add_argument("--itunes-cache", default=DEFAULT_ITUNES_CACHE, dest="itunes_cache", help="Cesta ku keši (iTunes)")
    ap.add_argument("--cache", dest="itunes_cache", help=argparse.SUPPRESS)  # alias pre starý názov
    # MusicBrainz cache a limity
    ap.add_argument("--mb-cache", default=DEFAULT_MB_CACHE, dest="mb_cache", help="Cesta ku keši (MusicBrainz)")
    ...
    ap.add_argument("--force", action="store_true", help="Ignoruj cache miss a hľadaj aj pre už vyplnené položky")
    args = ap.parse_args()
